...
class PaymentReportController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $summary = (clone $payments)->selectRaw('COUNT(*) as total_payments, COALESCE(SUM(payment_details.amount), 0) as total_amount, COALESCE(AVG(payment_details.amount), 0) as average_amount')->first();
        $methodSummary = (clone $payments)->selectRaw('payment_details.payment_method, COUNT(*) as total_payments, COALESCE(SUM(payment_details.amount), 0) as total_amount')->groupBy('payment_details.payment_method')->orderByDesc('total_amount')->get();
        $dailyPayments = (clone $payments)->selectRaw('DATE(transaction.created_at) as payment_date, COALESCE(SUM(payment_details.amount), 0) as total_amount')->groupBy('payment_date')->orderBy('payment_date')->get();
        $recentPayments = (clone $payments)
            ->select('payment_details.payment_method', 'payment_details.amount', 'payment_details.reference_number', 'transaction.id as transaction_id', 'transaction.created_at as transaction_date')
            ->orderByDesc('transaction.created_at')
            ->paginate(10)
            ->withQueryString();

        return view('reports.payments', compact('summary', 'methodSummary', 'dailyPayments', 'recentPayments', 'filter', 'startDate', 'endDate', 'start', 'end'));
    }
