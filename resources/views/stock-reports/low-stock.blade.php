    ...
    <main class="relative mx-auto max-w-7xl px-4 py-6 lg:px-8">
        <section class="mb-6 rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl shadow-black/30 backdrop-blur-xl">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <h1 class="mt-2 text-3xl font-semibold text-white md:text-4xl">Laporan stok produk.</h1>
                    <p class="mt-2 max-w-2xl text-sm text-slate-300">Pantau seluruh produk dan tandai item yang sudah mencapai batas minimum.</p>
                </div>
        ...
        <section class="mb-6 grid gap-4 md:grid-cols-3">
            <div class="rounded-3xl border border-white/10 bg-white/5 p-5 backdrop-blur-xl">
                <div class="text-sm text-slate-400">Total Produk</div>
                <div class="mt-2 text-3xl font-semibold text-white">{{ $summary['all'] }}</div>
                <div class="mt-1 text-xs text-slate-400">{{ $summary['total'] }} produk perlu perhatian</div>
            </div>
        ...
        <form method="GET" action="{{ route('stock-reports.low-stock') }}" class="mb-6 rounded-3xl border border-white/10 bg-white/5 p-5 backdrop-blur-xl">
            <label for="search" class="text-sm text-slate-300">Cari produk</label>
                        ...
                        <tbody class="divide-y divide-white/10 bg-slate-950/60">
                            @foreach ($products as $product)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-right font-semibold {{ $product->stock_status === 'habis' ? 'text-rose-300' : ($product->stock_status === 'menipis' ? 'text-amber-300' : 'text-emerald-300') }}">{{ $product->stock_quantity }}</td>
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        @else
                                            <span class="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-xs font-semibold text-emerald-200">Stok Penuh</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
