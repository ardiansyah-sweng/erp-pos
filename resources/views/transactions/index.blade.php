    ...
    <main class="relative mx-auto flex min-h-screen max-w-7xl flex-col gap-6 px-4 py-6 lg:px-8">
        @php
            ...
            $transactionPayload = $transactions->map(fn ($transaction) => [
                ...
                'parking_fee' => (function($payments) {
                    $payment = $payments->first();
                    if ($payment && isset($payment->parking_fee) && $payment->parking_fee > 0) {
                        return ['amount' => (int)$payment->parking_fee, 'label' => ucfirst($payment->parking_type ?? '')];
                    }
                    return null;
                ...
                'payments' => $transaction->payments->map(fn ($payment) => [
                    ...
                    'parking_fee' => $payment->parking_fee ?? 0,
                    'parking_type' => $payment->parking_type ?? 'none',
                ])->values(),
